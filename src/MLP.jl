...
model = Chain(Dense(28^2 => 32, sigmoid), Dense(32 => 10), softmax) |> gpu

p1 = model(rand(Float32, 28^2) |> gpu)  # run model on random data shaped like an image

@show sum(p1) ≈ 1;

p3 = model(rand(Float32, 28^2, 3) |> gpu)  # ...or on a batch of 3 fake, random "images"
...
function simple_loader(data::MNIST; batchsize::Int=64)
    ...
    Flux.DataLoader((x2dim, yhot); batchsize, shuffle=true) |> gpu
end
...
for epoch in 1:30
    ...
    for (x, y) in train_loader
        ...
        train_loss += l / length(train_loader)
    end
